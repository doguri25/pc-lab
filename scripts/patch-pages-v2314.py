...
css='''\n/* v2.3.14 — mobile tap-to-select and tap-slot install */\n@media (pointer:coarse){.catalog-mini-item.draggable-part{touch-action:manipulation!important;cursor:pointer!important}.catalog-mini-item.selected{box-shadow:0 0 0 2px color-mix(in srgb,var(--accent) 35%,transparent)!important}}\n'''
if css.strip() not in s:
    pos=s.find('</style>')
    if pos<0: raise SystemExit('style close missing')
    s=s[:pos]+css+s[pos:]

start=s.find('const data={"version":"2.3.13"')
end=s.find(';if(typeof module',start)
if start<0 or end<0: raise SystemExit('release data not found')
old_data=json.loads(s[start+len('const data='):end])
entry={
 'version':'2.3.14','date':'2026-09-14','timezone':'Asia/Seoul','channel':'stable-local',
 'title':'모바일 부품 선택 후 슬롯 터치 장착',
 'changes':[
  '모바일 부품 목록에서는 손가락 드래그를 시작하지 않고 부품을 한 번 터치해 선택하는 방식으로 변경하여 세로 스크롤과 부품 선택이 서로 충돌하지 않도록 수정',
  '선택한 부품이 있는 상태에서 메인보드·CPU·쿨러·GPU·저장장치·파워·네트워크 등 호환되는 빈 장착 슬롯을 한 번 터치하면 즉시 장착 절차가 실행되도록 변경',
  'RAM은 기존 RAM 슬롯 구역 확대 흐름을 유지하면서 RAM 선택 후 원하는 빈 슬롯을 터치하면 바로 장착되도록 통일',
  "모바일에서 선택한 부품은 목록과 상세 패널에서 선택 상태를 명확히 표시하고, 안내 문구를 '부품 선택 → 슬롯 터치' 순서로 변경",
  '데스크톱의 기존 드래그 장착 기능과 고정 해제 후 부품을 분리 작업대로 드래그해 빼는 분해 조작은 그대로 유지'
 ],
 'scopeNote':'모바일 조립 입력 방식을 터치 선택 중심으로 단순화하며 데스크톱 드래그 조작과 기존 조립·호환성 규칙은 유지합니다.'
}
new_data={**entry,'buildId':'2.3.14-96bd6394e936','history':[entry,*old_data['history']]}
s=s[:start]+'const data='+json.dumps(new_data,ensure_ascii=False,separators=(',',':'))+s[end:]
p.write_text(s)
